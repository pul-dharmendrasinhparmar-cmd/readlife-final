...
export default function BookwormDetailPage() {
  const [profile, setProfile] = useState<GameProfile | null>(null);
  const [open, setOpen] = useState(false);
  const game = MINI_GAMES.find((g) => g.id === "bookworm") as MiniGame;
    ...
    <div className="page-shell min-h-screen pb-16">
      ...
      <main className="mx-auto max-w-3xl px-4 pt-8 sm:px-6">
        ...
        <p className="mt-3 max-w-xl text-muted">
          Guide your bookworm through the library, collect books, and beat your
          high score.
        </p>
        <button
          type="button"
            ...
            <ul className="mt-4 grid gap-3 sm:grid-cols-2 text-sm text-muted">
              <li>
                ...
                <p className="font-serif text-2xl font-semibold text-ink">
                  {w.personalBest.toLocaleString()}
                </p>
              </li>
              <li>
                <span className="text-muted-soft">Last run</span>
                <p className="font-semibold text-ink">
                  {w.lastScore?.toLocaleString() ?? "—"}
                </p>
              </li>
              <li>
                <span className="text-muted-soft">Highest level</span>
                <p className="font-semibold text-ink">
                  {w.highestLevelReached}
                </p>
              </li>
              <li>
                <span className="text-muted-soft">Books collected</span>
                <p className="font-semibold text-ink">
                  {w.totalBooksCollected.toLocaleString()}
                </p>
              </li>
              <li>
                <span className="text-muted-soft">Games played</span>
                <p className="font-semibold text-ink">{w.gamesPlayed}</p>
              </li>
              <li>
                <span className="text-muted-soft">Play streak</span>
                <p className="font-semibold text-ink">
                  {w.currentPlayStreak} days
                </p>
              </li>
            </ul>
          </section>
