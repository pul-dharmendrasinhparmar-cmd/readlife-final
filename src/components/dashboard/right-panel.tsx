...
export function RightPanel({
  ...
  return (
    <aside className="flex flex-col gap-3.5 lg:max-h-[calc(100vh-1.5rem)] lg:overflow-y-auto lg:pr-1">
      {/* Currently reading */}
      <section className="rounded-[1.35rem] border border-[#e4d5c3] bg-[#fbf6ee] p-4 shadow-[0_6px_20px_rgba(60,45,30,0.05)] sm:p-5">
        <h2 className="text-[0.68rem] font-semibold tracking-[0.14em] text-forest/70 uppercase">
          Currently Reading
        </h2>

        {!current ? (
          <div className="mt-3 rounded-2xl border border-dashed border-[#dccab4] bg-[#f7f0e6] px-4 py-5 text-center">
            <p className="text-sm text-muted">Nothing in progress.</p>
            <Link
              href="/library"
              className="mt-2 inline-block text-sm font-semibold text-forest underline-offset-2 hover:underline"
            >
              Pick from Library →
            </Link>
          </div>
          ...
          <>
            <div className="mt-3 flex gap-3.5">
              <div
                className="relative h-[118px] w-[78px] shrink-0 overflow-hidden rounded-lg shadow-[0_8px_18px_rgba(40,30,20,0.18)]"
              ...
              <div className="min-w-0 flex-1 pt-0.5">
                <p className="font-serif text-[1.05rem] leading-snug font-semibold text-forest">
                  {current.book.title}
                </p>
            ...
            {featuredQuote ? (
              <div className="mt-3.5 flex gap-2 rounded-xl bg-[#f0e6d6]/90 px-3 py-2.5">
                <LeafIcon className="mt-0.5 h-3.5 w-3.5 shrink-0 text-forest-soft" />
                <p className="text-[0.78rem] leading-snug text-forest/80 italic">
                  “{featuredQuote}”
                </p>
              </div>
            ) : null}
            ...
            <div className="mt-4 flex gap-2">
              ...
              <div className="relative" ref={menuRef}>
                <button
                  ...
                  className="flex h-full min-w-[2.75rem] items-center justify-center rounded-full border border-[#dccab4] bg-[#f7f0e6] text-forest transition hover:bg-[#efe4d4]"
                >
                  ···
                </button>
                {menuOpen ? (
                  <div className="absolute right-0 bottom-full z-20 mb-2 w-44 overflow-hidden rounded-2xl border border-[#e4d5c3] bg-[#fbf6ee] shadow-lg">
                    <button
                      type="button"
                      className="block w-full px-3.5 py-2.5 text-left text-sm text-forest hover:bg-[#efe4d4]"
                      onClick={() => {
                        setMenuOpen(false);
                        onUpdateProgress();
                      }}
      ...
      <section className="rounded-[1.35rem] border border-[#e4d5c3] bg-[#fbf6ee] p-4 shadow-[0_6px_20px_rgba(60,45,30,0.05)] sm:p-5">
              ...
              >
                <span
                  className={`flex h-8 w-8 items-center justify-center rounded-full text-[0.7rem] font-bold ${
                      ...
                      : "border border-[#dccab4] bg-transparent text-muted-soft"
                  }`}
                >
                  {done || today ? "✓" : ""}
                </span>
        ...
        <div className="mt-5 border-t border-[#e8dccb] pt-4">
          ...
          <div className="mt-2 flex items-end justify-between gap-3">
            ...
            <PlantIcon className="h-8 w-8 text-forest-soft" />
          </div>
          <div className="mt-3">
            <div className="mb-1.5 flex justify-between text-[0.72rem] text-muted">
              <span>
              ...
              <span className="font-semibold text-forest">{todayPct}%</span>
            </div>
          ...
          {actions.map((action) => {
            ...
            return (
              <button
                ...
                className="flex flex-col items-center gap-2 rounded-xl px-1 py-2 text-center transition hover:bg-[#f0e6d6]"
              >
                <span className="flex h-11 w-11 items-center justify-center rounded-full border border-[#e0d1bf] bg-[#f7f0e6] text-forest shadow-sm">
                  <Icon className="h-4 w-4" />
                </span>
                <span className="text-[0.62rem] leading-tight font-semibold text-forest">
                  {action.label}
                </span>
              </button>
            );
        ...
        {upcoming.length === 0 ? (
          ...
          <ul className="mt-3 space-y-2.5">
            {upcoming.map((u) => (
              <li
                key={u.id}
                className="flex items-center gap-3 rounded-xl bg-[#f0e6d6]/80 px-3 py-2.5"
              >
                {u.bookCover ? (
                  <span
                  ...
                  <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-[#efe4d4] text-forest">
      ...
      <section className="rounded-[1.35rem] border border-[#e4d5c3] bg-[#fbf6ee] p-4 shadow-[0_6px_20px_rgba(60,45,30,0.05)] sm:p-5">
        ...
        <p className="mt-2 text-sm leading-snug text-forest/80">
          You&apos;ve finished{" "}
        ...
        <Link
          href="/insights"
          className="mt-4 inline-block text-sm font-semibold text-forest underline-offset-2 hover:underline"
        >
          View Insights →
        </Link>
      </section>
      ...
      {readingEra ? (
        <section className="rounded-[1.35rem] border border-[#e4d5c3] bg-[#fbf6ee] p-4 shadow-[0_6px_20px_rgba(60,45,30,0.05)] sm:p-5">
          ...
          <p className="mt-2 font-serif text-lg font-semibold text-forest">
            {readingEra.title}
          </p>
