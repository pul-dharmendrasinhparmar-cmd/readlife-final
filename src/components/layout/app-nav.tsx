...
export function AppNav() {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-[#e4d5c3]/80 bg-[#f7f0e6]/95 backdrop-blur-md">
      <div className="mx-auto flex h-[4.25rem] max-w-[1440px] items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
        <Link href="/" className="flex shrink-0 items-center gap-2 text-forest">
            ...
            return (
              <Link
                ...
                className={`relative flex items-center gap-2 rounded-full px-3.5 py-2 text-sm font-medium transition ${
                  active
                    ? "text-forest"
                    : "text-forest/65 hover:bg-[#efe4d4] hover:text-forest"
                }`}
        ...
        <div className="flex items-center gap-3">
          <button
            type="button"
            className="relative flex h-10 w-10 items-center justify-center rounded-full text-forest transition hover:bg-[#efe4d4]"
            aria-label="Notifications"
          >
            <BellIcon className="h-5 w-5" />
            <span className="absolute top-1.5 right-1.5 h-2 w-2 rounded-full bg-[#d45545] ring-2 ring-[#f7f0e6]" />
          </button>
          <Link
            href="/profile"
            className="relative h-10 w-10 overflow-hidden rounded-full border border-[#dccab4] bg-[#efe4d4]"
